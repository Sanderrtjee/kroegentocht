...
import { formatDate, formatDistance, formatRating, formatTime, stars } from '../lib/format.js';
import {
  Badge,
  Button,
  Card,
...
export function CrawlsPage() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-semibold text-nacht-200">Kroegentochten</h1>

      <Card>
        <h2 className="mb-3 font-medium text-nacht-200">Nieuwe tocht</h2>
        ...
        {crawls.data?.items.map((crawl) => (
          <Card key={crawl.id}>
            <div className="flex flex-wrap items-baseline justify-between gap-2">
              <Link
                to={`/tochten/${crawl.id}`}
                className="font-medium text-nacht-200 hover:underline"
              >
                {crawl.name}
              </Link>
              <span className="text-sm text-nacht-400">{formatDate(crawl.crawlDate)}</span>
            </div>
            <div className="mt-1 flex flex-wrap gap-3 text-sm text-nacht-400">
              <span>
...
export function CrawlDetailPage() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <div>
          <h1 className="text-xl font-semibold text-nacht-200">{detail.name}</h1>
          <p className="text-sm text-nacht-400">{formatDate(detail.crawlDate)}</p>
        </div>
        <div className="flex flex-wrap gap-3 text-sm text-nacht-400">
      ...
      {detail.attendees.length > 0 ? (
        <Card>
          <h2 className="text-sm font-medium text-nacht-200">Wie er die avond bij waren</h2>
          <p className="mt-1 text-sm text-nacht-400">{detail.attendees.join(', ')}</p>
        </Card>
      ) : null}
      ...
      <ol className="space-y-2">
        {detail.stops.map((stop, index) => (
          <li key={stop.visit.id}>
            {index > 0 ? (
              <div className="flex items-center gap-2 py-1 pl-4 text-xs text-nacht-400">
                <span className="h-6 w-px bg-nacht-700" />
                {formatDistance(stop.distanceFromPrevM)} verder
              </div>
            ) : null}
            <Card className="flex gap-3">
              <div className="grid size-8 shrink-0 place-items-center rounded-full bg-nacht-700 text-sm font-semibold">
                {index + 1}
              </div>
              {stop.visit.photos[0] ? (
                <img
              ...
              <div className="min-w-0 flex-1">
                <div className="flex flex-wrap items-baseline gap-2">
                  ...
                  <span className="text-bier-400">{stars(stop.visit.rating)}</span>
                ...
                {stop.visit.attendees.length > 0 ? (
                  <p className="mt-1 text-xs text-nacht-400">
              ...
              <div className="flex flex-col gap-1">
                <button
                  type="button"
                  className="rounded px-2 text-nacht-400 hover:bg-nacht-800 disabled:opacity-30"
                ...
                <button
                  type="button"
                  className="rounded px-2 text-nacht-400 hover:bg-nacht-800"
                  onClick={() =>
      ...
      {adding ? (
        <Card>
          <h2 className="mb-2 font-medium text-nacht-200">Bezoek toevoegen</h2>
      ...
      <p className="text-xs text-nacht-600">
